...
# Stephanie Galvan
# Parallel Computing 4:30PM - 5:50PM
...
def multiply_matrix(mat_a, mat_b, total_threads):
	"""
	Function that receives two matrices and the number of threads to be run in
	"""
    ...
    with pymp.Parallel(total_threads) as p:
        ...
        for i in p.range(len(mat_a)):
            for j in p.range(len(mat_b[0])):
                for k in p.range(len(mat_b)):
                    matrix[i][j] += ((mat_a[i][k] * mat_b[k][j]) * total_threads)
    return matrix
